...
int main()
{
    ...
    for (int i = 0; i < 12; i++)
        days[i] = new Day[potok_cnt];

    for (int i = 0; i < 12; i++)
        for (int j = 0; j < potok_cnt; j++)
        {
    ...
    for (int i = 0; i < dis + 1; i++)
    {
        ...
        {
            int flag = 0;
            for (int k = 0; k < potok_cnt; k++)
            {
                for (int a = 0; a < days[d][k].lessons_count; a++)
                    if (disciples[i].sem_name[j] == days[d][k].name[a] || disciples[i].sem_aud[j] == days[d][k].aud[a])
                        flag++;
            }
    ...
    for (int i = 0; i < dis + 1; i++)
    {
        for (int j = 0; j < disciples[i].lab_count; j++)
        {
            ...
            {
                for (int a = 0; a < days[d][k].lessons_count; a++)
                    if (disciples[i].lab_name[j] == days[d][k].name[a] || disciples[i].lab_aud[j] == days[d][k].aud[a])
                        flag++;
            }
            if ((days[d][p].lessons_count < 4) && (!flag))
            {
